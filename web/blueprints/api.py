...
async def reactionrole_add(request, guild, message, emoji):
	...
	check = request.app.ctx.db.query_row("SELECT enabled FROM reactionrole WHERE guild = ? AND emoji_id = ? AND message_id = ?", guild, emoji, message)

	if not check:
		request.app.ctx.db.execute("INSERT INTO reactionrole (guild, message_id, emoji_id, role_id, enabled) VALUES (?, ?, ?, ?, ?)", guild, message, emoji, _json["op"], 1)
...
async def reactionrole_toggle(request, guild, message, emoji):
	...
	if not check:
		...
		request.app.ctx.db.execute("UPDATE reactionrole SET enabled = 1 WHERE guild = ? AND emoji_id = ? AND message_id = ?", guild, emoji, message)
	elif check == 1:
		request.app.ctx.db.execute("UPDATE reactionrole SET enabled = 0 WHERE guild = ? AND emoji_id = ? AND message_id = ?", guild, emoji, message)
...
async def reactionrole_update_role(request, guild, message, emoji):
	...
	request.app.ctx.db.execute("UPDATE reactionrole SET role = ? WHERE guild = ? AND emoji_id = ? AND message_id = ?", _json["op"], guild, emoji, message)
