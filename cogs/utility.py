...
class Utility(cogs.Cog):
	...
	async def botclear(self, bot, ctx, limit: int = 50):
		...
		await permissions.check_permissions(ctx, manage_messages=True)
		
		if limit > 500:
			return await ctx.send("Please choose a number lower than 500.")
		conf = await ctx.prompt(f"Are you sure? this will clear the last {limit} messages of bots, you have 10 seconds to confirm or deny.", delete_after=True, embed=True, author_id=ctx.author.id, timeout=10)
		if conf:
			try:
	...
	async def purge(self, bot, ctx, limit: int = None):
		...
		if limit > 500:
			return await ctx.send("The limit on purgable messages is set to 500 due to lag with logging. (if you have it on, and are constantly purging, consider turning logs off before doing so, `(prefix)log false`)")
		msg = await ctx.send(f"Trying to purge {limit} messages.")
		await ctx.message.delete()
		await msg.delete()
