...
class CoreEvents(cogs.Cog):
	...
	async def on_command_error(self, ctx, err):
		if isinstance(err, CantReachAPI) or isinstance(err, ConnectionRefusedError):
			...
			return await ctx.send("Either you, or i, do not have the permissions to do this, please make sure we both have the relevent permissions for the command.")
		...
		elif not self._is_main_instance:
			self.ipc.error(f"New uncaught error in {ctx.guild.name} ({ctx.guild.id}):\n{err}")
		raise err

	@events.register()
	async def on_overload_notice(self, guild: discord.Guild, channel: discord.TextChannel):
		return await channel.send("The bot has detected a raid/massive influx of messages, and is temporarily shutting down for this guild.\nPlease enable the `panic` feature to enable a system like this for anti-raid protection.")
		# TODO: stuff
