...
from lana.utils import checks, default, permissions
from lana.utils.predefiner import PredefinedActions
...
class Moderation(cogs.Cog):
    ...
    async def kick(self, bot, ctx, member: discord.Member, reason: str = None):
        ...
        await permissions.check_permissions(ctx, bot.config.owners, kick_members=True)
    ...
    async def nickname(self, bot, ctx, member: discord.Member, name: str = None):
        ...
        await permissions.check_permissions(ctx, bot.config.owners, manage_nicknames=True)
        member = await bot.converter.member(ctx, member)
    ...
    async def mute(self, bot, ctx, member: discord.Member, reason: str = None):
        ...
        predefined.make_collection(
            ...
            (
                mute_role.name if mute_role else None
            ),  # If this is None then role was not found and it will look for an action with the trigger None.
        ).add_action(
            trigger=(
                mute_role if mute_role else 0
            ),  # Set the trigger to the role name if found, otherwise default to 0 to make inaccessible (mute_role cannot ever return 0).
            action=[member.add_roles(mute_role, reason=default.responsible(ctx.author, reason)), ctx.send(default.actionmessage("muted"))],
        ).add_action(
    ...
    async def ban(self, bot, ctx, member: discord.Member = None, reason: str = None):
        ...
        await permissions.check_permissions(ctx, bot.config.owners, ban_members=True)
        if checks.strict_actions(ctx):
            checks.is_known_mod(ctx, ctx.author.id)
        if member is None:
        ...
        for member_id in members:
            try:
                ...
                return await ctx.send(
                    "There was in invalid ID. Could not convert to integer, make sure all the IDs only have numbers in them."
                )
    ...
    async def decancer(self, bot, ctx, member: discord.Member):
        ...
        if (
            ctx.me.permissions_in(ctx.channel).manage_nicknames
            and ctx.author.permissions_in(ctx.channel).manage_nicknames
            and not checks.strict_actions(ctx)
        ):
            if checks.strict_actions(ctx):
                checks.is_known_mod(ctx, ctx.author.id)
            ...
            try:
                ...
                await ctx.send(
                    "I dont have the correct permissions to do this, make sure i have `Manage Nicknames` or that my role is higher than the member's highest role"
                )
        else:
            cancer = member.display_name
            decancer = unidecode.unidecode_expect_nonascii(cancer)
...
def export(bot):
    exports = {"cog": Moderation(bot), "name": "Moderation"}
    return exports
