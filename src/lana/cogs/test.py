...
from lana.utils import permissions, redis
...
class Test(cogs.Cog):
    ...
    async def predtest(self, bot, ctx):
        ...
        predefined.make_collection(trigger=True, to_exec=second_prompt).add_action(True, ctx.send("But are you really sure?")).add_action(
            False, ctx.send("Then you should be extra sure.")
        )
...
def export(bot):
    exports = {"cog": Test(bot), "name": "Testing"}
    return exports
