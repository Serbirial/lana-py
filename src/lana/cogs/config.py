...
class Config(cogs.Cog):
    ...
    async def prefixremove(self, bot, ctx, prefix: str):
        ...
        connection = (
            api.InternalApiConnection(ctx, URI)
            .expect_status_codes([200])
            .set_default_action(ctx.send("The API sent back an un-expected response."))
        )
        resp = (
            (await connection.post(require_json=True, json={"op": prefix.strip()}))
            .expect_json_key("op")
            .expect_json_value("op", True, True)
        )
    ...
    async def panictoggle(self, bot, ctx):
        ...
        connection = (
            api.InternalApiConnection(ctx, URI)
            .predefine_json_actions("op", actions)
            .expect_status_codes([200])
            .set_default_action(ctx.send("The API sent back an un-expected response."))
        )
    ...
    async def antialtdays(self, bot, ctx, time_in_days: int):
        ...
        actions = {-1: ctx.send("Error while converting to integer."), True: ctx.send(f"Time has been set to {days}.")}
...
def export(bot):
    exports = {"cog": Config(bot), "name": "Settings"}
    return exports
