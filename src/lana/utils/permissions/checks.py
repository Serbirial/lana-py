from exceptions import CommandCheckError, PremiumCheckError


async def check_permissions(ctx, owners, **perms):
    if ctx.author.id in owners:
        return True

    resolved = ctx.channel.permissions_for(ctx.author)
    ret = all(getattr(resolved, name, None) == value for name, value in perms.items())
    if ret != True:
        raise CommandCheckError("Command check did not pass.")


def owner(ctx, owners):
    return ctx.author.id in owners


def is_owner(ctx, owners):
    return owner(ctx, owners)
