...
class LanaAR(AutoShardedClient):
    ...
    async def on_ready(self):
        ...
        try:
            await self.change_presence(
                status=discord.Status.online, activity=discord.Game(f"with {len(list(self.get_all_members()))} foxes | lana help")
            )
    ...
    async def get_prefix(self, message: discord.Message) -> list | str:
        ...
        prefixes = [
            DEFAULT_PREFIX
        ]  # TODO: sort prefixes while adding to mariadb instead of relying on intensive sorting every new message.
        try:
